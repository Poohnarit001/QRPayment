...
const IMG_RESIZE_FIT = "cover"; // cover = เต็มจอแต่มีสิทธิ์โดน crop, contain = เห็นครบแต่มีขอบ
...
const imageUpload = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: 8 * 1024 * 1024 }, // allow larger source image, then auto-resize before save
  fileFilter: (req, file, cb) => {
...
app.post("/api/images/upload", imageUpload.single("image"), async (req, res) => {
  try {
    ...
    if (!req.file?.buffer?.length) return res.status(400).json({ ok: false, error: "image required" });

    ensureDirSync(IMG_DIR);

    const filename = device_id === "ALL" ? "splash.jpg" : `splash_${device_id}.jpg`;
    ...
    const st = fs.existsSync(full) ? fs.statSync(full) : { size: 0 };
    ...
    return res.json({
      ...
      url,
      size: st.size || 0,
      width: meta?.width || IMG_WIDTH,
      height: meta?.height || IMG_HEIGHT,
      fit: IMG_RESIZE_FIT,
      updated_at: Date.now()
    });
  } catch (e) {
